...
use crate::cube_publisher::cube_publisher;
    ...
    F: FnOnce(SocketAddrV4) + Send + 'static,
{
    ...
    let (tx_cube, rx_cube) = mpsc::unbounded_channel();
    ...
    let cube_handle = tokio::spawn(async move {
        cube_publisher(
            rx_cube,
            cube_login_url,
            cube_chris_username,
    ...
    let result = tokio::try_join!(
        ...
        messenger(rx_register, &tx_lonk, &tx_cube).map_err(anyhow::Error::from)
    );
